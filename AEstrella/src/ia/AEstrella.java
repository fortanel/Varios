...
//        mapa.mundo[4][11].tipo = Tipo.OBSTACULO;// Lo agrege
//        mapa.mundo[5][11].tipo = Tipo.OBSTACULO;
//        mapa.mundo[6][11].tipo = Tipo.OBSTACULO;
//        mapa.mundo[3][11].tipo = Tipo.OBSTACULO;
//        mapa.mundo[2][11].tipo = Tipo.OBSTACULO;
    ...
    class Mosaico{
        ...
        void calculaHeuristica(Mosaico meta){
            ...
            */
            ...
            hn =  (Math.abs(renglon - meta.renglon) + Math.abs(columna - meta.columna)); //Sacamos diferencias entre las coordenadas.
            hn = hn * 10;
        }
        ...
        LinkedList<NodoBusqueda> getSucesores() {
            ...
            for(Accion a : Accion.values()) {
                ...
                if(sucesor != null) {
                    ...
                    if(sucesor.padre == null || sucesor.gn > nodoSucesor.gn ){
                        sucesor.padre = estado;
                    }
...
//             aqui deben incializar sus listas abierta y cerrada
        ...
        void expandeNodoSiguiente() {
            /**
              ...
              */
            LinkedList<NodoBusqueda> sucesores;
             // En caso de se llegue al objetivo el algoritmo termina.
            if (!resuelto){
                if(nodoActual != null){ // Cambiamos la situacion del ultimo nodo en la lista cerrada
                    nodoActual.estado.situacion = Situacion.EN_LISTA_CERRADA;
                }
                
                nodoActual = listaAbierta.poll(); //Sacamos al ultimo nodo de la lista abierta.
                nodoActual.estado.situacion = Situacion.ACTUAL; 
                listaCerrada.put(nodoActual.estado,nodoActual.estado); //Ingresamos el nodo actual a la lista cerrada.
                
                if(nodoActual.estado == estadoFinal){ // Verificamos si llegamos al destino
                    resuelto = true; // Marcamos que el algoritmo obtuvo el resultado esperado
                    while(nodoActual.padre != null){ //Obtenemos la solucion.
                    nodoActual.estado.situacion = Situacion.EN_SOLUCION;
                    nodoActual = nodoActual.padre;
                    }
                    return;
                }
                sucesores = nodoActual.getSucesores(); // En caso de que no estemos en la solucion obtenemos los vecinos del nodo actual.
                for(NodoBusqueda nodo: sucesores){ //Iteramos por cada uno de los vecinos.
                    if(!listaCerrada.containsKey(nodo.estado)){
                        if(!listaAbierta.contains(nodo)){ // Si no se encuentra en la lista abierta
                            nodo.estado.calculaHeuristica(estadoFinal);//Calculamos la heuristica.
                            nodo.estado.gn = nodo.gn; //Actualizamos el valor del estado de gn.
                            listaAbierta.add(nodo); //Agremos el nodo a la lista abierta
                            nodo.estado.situacion = Situacion.EN_LISTA_ABIERTA;
                        } else if ( nodo.gn < nodo.estado.gn){ // Notese que nodo.gn tiene
                            listaAbierta.remove(nodo); //Retiramos de la lista abierta                          
                            nodo.padre = nodoActual; // Cambiamos el nodo padre.
                            nodo.estado.padre = nodoActual.estado; //Cambiamos el estado del nodo
                            nodo.estado.gn = nodo.gn; //Cambiamos la gn.
                            nodo.gn = nodo.estado.gn; // Cambiamos nodo.gn
                            listaAbierta.offer(nodo); //Volvemos a ingresar el valor.
                        }
                    }
                } 
            } 
